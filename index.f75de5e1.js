...
function salaryToText(salary) {
    return "$" + salary.toString().split("").reduce((text, char, index)=>text + char + ((salary.toString().length - index - 1) % 3 === 0 ? "," : ""), "").slice(0, -1);
}
...
function areInputsValid(employeeName, position, office, age, salary) {
    for (const arg of arguments)if (isInputEmpty(arg.value)) {
...
function getComparator(targetHeaderText) {
    switch(targetHeaderText){
        ...
        case "Salary":
            return (firstPerson, secondPerson)=>firstPerson.salary - secondPerson.salary;
    }
}
function sortTable() {
...
function selectRow() {
    ...
    for(let i = 0; i < rowsAfterActivation.length; i++)if (rowsAfterActivation[i].matches(".active")) people[i].active = true;
}
function saveCell(e) {
...
function addPerson(e) {
    const [employeeName, position, age, salary] = document.querySelectorAll("label > input");
...
rows.forEach((row)=>{
    ...
    for (const cell of row.children)cell.addEventListener("dblclick", editCell);
    row.addEventListener("click", selectRow);
});
